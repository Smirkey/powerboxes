...
use nalgebra::{MatrixXx1, MatrixXx4, Scalar, ClosedMul};
#[derive(Copy, Clone)]
pub enum BoxFormat {
    XYXY,
    XYWH,
...
/// # Examples
/// ```
/// use nalgebra::{MatrixXx4, MatrixXx1};
/// use powerboxesrs::boxes::box_areas_nalgebra;
/// let boxes = MatrixXx4::from_vec(vec![1., 0., 2., 0., 3., 10., 4., 10.]);
///
/// let areas = box_areas_nalgebra(boxes);
/// assert_eq!(areas, MatrixXx1::from_vec(vec![4., 100.]));
/// ```
pub fn box_areas<N>(boxes: &MatrixXx4<N>) -> MatrixXx1<N>
where
    N: Scalar + Num + SubAssign + ClosedMul,
{
    let widths = boxes.column(2) - boxes.column(0);
    let heights = boxes.column(3) - boxes.column(1);
    let areas = widths.component_mul(&heights);
    return areas;
}
...
/// ```
// pub fn remove_small_boxes<N>(boxes: &MatrixXx4<N>, min_size: f64) -> MatrixXx4<N>
// where
// N: Scalar + Num + SubAssign + ClosedMul + ToPrimitive,
// {
//     let areas = box_areas(boxes);
//     let keep = areas.map(|x| x.to_f64().unwrap() > min_size);
//     let indices: Vec<usize> = keep.iter().enumerate().filter_map(|(i, x)| if *x { Some(i) } else { None }).collect();
//     return boxes.select_rows(&indices);
// }
pub fn remove_small_boxes<N>(boxes: &MatrixXx4<N>, min_size: f64) -> MatrixXx4<N>
where
    N: Scalar + Num + SubAssign + ClosedMul + ToPrimitive + Copy,
{
    // Get the areas of the boxes
    let areas = box_areas(boxes);

    // Collect indices of boxes whose areas are larger than the min_size
    let indices: Vec<usize> = areas.iter()
        .enumerate()
        .filter_map(|(i, &area)| {
            // Convert to f64 safely
            area.to_f64().and_then(|a| if a > min_size { Some(i) } else { None })
        })
        .collect();

    // Select rows (boxes) that pass the filter
    boxes.select_rows(&indices)
}
